...
export default function Home() {
  ...
  // This will be used for displaying the base URL for path-based links if not a custom domain.
  const displayBaseDomain = process.env.NEXT_PUBLIC_ROOT_DOMAIN || (process.env.VERCEL_URL || 'localhost:9002');
    ...
    <div className="flex flex-col items-center text-center space-y-16">
      <section className="space-y-6 max-w-3xl pt-8">
        ...
        <p className="text-lg md:text-xl text-muted-foreground leading-relaxed">
          ...
          all seamlessly hosted. Get started for free and elevate your client offerings.
        </p>
         <p className="text-sm text-muted-foreground">
          (When using Vercel's default domain, subdomains are path-based: {displayBaseDomain}/[client_id])
        </p>
      </section>

      <section className="grid md:grid-cols-2 gap-8 w-full max-w-4xl">
        {clients.map((client) => {
          const clientPathUrl = `/${client.id}`;
          const clientSubdomainStyleUrl = getCanonicalClientUrl(client.id, '/');
          const displaySubdomainStyleUrl = clientSubdomainStyleUrl.replace(/^https?:\/\//, '');
          ...
          return (
            <Card key={client.id} className="shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg flex flex-col">
              ...
              <CardContent className="space-y-3 mt-auto">
                <Link href={clientPathUrl} passHref>
                  <Button variant="default" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground transition-all duration-300 ease-in-out transform hover:scale-105">
                    <Link2 className="mr-2 h-5 w-5" /> View (Path: {clientPathUrl})
                  </Button>
                </Link>
                <Link href={clientSubdomainStyleUrl} passHref target="_blank" rel="noopener noreferrer">
                  <Button variant="outline" className="w-full border-primary text-primary hover:bg-primary/10 hover:border-primary/80 transition-all duration-300 ease-in-out transform hover:scale-105">
                    <Globe className="mr-2 h-5 w-5" /> View ({displaySubdomainStyleUrl})
                  </Button>
                </Link>
              </CardContent>
            </Card>
      ...
      <section className="w-full max-w-3xl space-y-8 py-12">
        ...
        <ul className="space-y-4">
          {[
            ...
            { title: "Free Hosting Options", description: "Utilize Vercel's generous free tier for hosting." },
